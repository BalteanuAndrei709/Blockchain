...
contract ProductDeposit {
    ...
    function retrieveProduct(uint256 productId, uint256 amount) external returns(bool){
        require(
...
contract ProductStore {
    ...
    mapping(uint => Product) public myStore; //idProdus => product
    ...
    function addProductIntoStore(uint256 productId, uint256 quantity) external {
        ProductDeposit(productDepositContract).retrieveProduct(productId, quantity);
        //add product to store
        myStore[productId].quantity = myStore[productId].quantity + quantity; 
    }

    function setPricePerUnit(uint productId, uint price ) external {
        require(myStore[productId].quantity == 0, "This product does not exists!");
        myStore[productId].pricePerUnit = price;
    }

    function checkAvailabilityAndAuthenticity(uint productId) 
    external 
    view
    returns(bool) {
        ...
        address producerOfProduct = ProductIdentification(productIdentificationContract).isProductRegistered(productId).producer;
        if(!ProductIdentification(productIdentificationContract).isProducerRegistered(producerOfProduct))
        {
            return false;
        }
        return true;
    ...
    function buyProduct(uint productId, uint units) 
    external
    payable {
        require(myStore[productId].quantity - units >= 0, "This product is out of availability!");
        require(msg.value >= myStore[productId].pricePerUnit * units, "Not enough money!");
        myStore[productId].quantity = myStore[productId].quantity - units;
        address producerOfProduct = ProductIdentification(productIdentificationContract).isProductRegistered(productId).producer;
        payable(producerOfProduct).transfer(units *  myStore[productId].pricePerUnit / 2);
